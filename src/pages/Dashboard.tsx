...
export const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans selection:bg-black selection:text-white overflow-hidden flex">
      ...
      {/* Sidebar Navigation */}
      <aside className="w-80 h-screen border-r border-black/5 flex flex-col p-10 bg-white/50 backdrop-blur-xl sticky top-0 shrink-0">
        <div className="mb-20">
          <Link to="/" className="flex items-center gap-4 group">
            <div className="w-14 h-14 bg-black rounded-[1.5rem] flex items-center justify-center rotate-3 group-hover:rotate-0 transition-all duration-700 shadow-2xl">
            ...
            <div>
              ...
              <p className="text-label text-[8px] mt-1 tracking-[0.4em]">IT ELITE ECOSYSTEM</p>
            </div>
          </Link>
        </div>
        ...
        <div className="space-y-6">
          <div className="premium-card p-6 border-none bg-black/5 flex items-center gap-4">
             <div className="w-12 h-12 bg-black rounded-2xl flex items-center justify-center text-white font-black text-xl rotate-3">
               {user.fullName[0]}
             </div>
             <div className="overflow-hidden">
                <p className="text-display text-sm truncate">{user.fullName}</p>
                <p className="text-label text-[7px] mt-0.5">{user.role}</p>
             </div>
          </div>
          <button onClick={logout} className="w-full flex items-center gap-4 px-8 py-5 text-gray-400 hover:text-black hover:bg-black/5 rounded-3xl transition-all duration-500">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        {/* Modern Header */}
        <header className="h-32 border-b border-black/5 flex items-center justify-between px-16 bg-white/30 backdrop-blur-md sticky top-0 z-50">
          <div className="flex flex-col">
             ...
             <div className="flex items-center gap-4 mt-1">
                <span className="text-label">{user.role === 'client' ? 'Client Authority' : 'Specialist Access'}</span>
                <span className="w-1.5 h-1.5 bg-black/10 rounded-full"></span>
                <span className="text-label text-black/20">{new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}</span>
             </div>
          </div>
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="flex items-center gap-3">
               <button className="w-12 h-12 rounded-full border border-black/5 flex items-center justify-center hover:bg-black hover:text-white transition-all duration-500"><Bell className="w-4 h-4" /></button>
               <button className="w-12 h-12 rounded-full border border-black/5 flex items-center justify-center hover:bg-black hover:text-white transition-all duration-500"><Globe className="w-4 h-4" /></button>
            </div>

            <Link to="/" className="btn-lux px-8 py-4 text-[10px] shadow-sm bg-zinc-50 text-black border border-black/5 hover:bg-black hover:text-white transition-all duration-700">
           ...
           <AnimatePresence mode="wait">
               ...
               >
                 <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="text-label flex items-center gap-2">
                       <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div> {filteredJobs.length} Live Opportunities
                    </div>
                 </div>

                 {filteredJobs.length === 0 ? (
                   <div className="py-40 flex flex-col items-center justify-center premium-card border-dashed">
                     <Layers className="w-20 h-20 text-black/10 mb-6" />
               ...
               >
                 {user.role === 'client' && (
                   <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                     <div className="lg:col-span-2 premium-card p-12 bg-black text-white relative overflow-hidden group">
                        <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20 group-hover:scale-110 transition-transform duration-[20s]"></div>
                        <div className="relative z-10 flex flex-col justify-between h-full space-y-10">
                           <div className="space-y-4">
                              <h2 className="text-display text-7xl leading-none">Scale Your Vision</h2>
                              <p className="text-zinc-400 text-sm max-w-md font-medium italic">Broadcast your requirements to the elite specialist network and begin procurement in minutes.</p>
                           </div>
                           <button 
                             onClick={() => setIsCreatingJob(true)}
                             className="w-fit flex items-center gap-6 px-10 py-6 bg-emerald-500 text-white rounded-full font-black uppercase tracking-[0.2em] text-xs hover:bg-white hover:text-black transition-all duration-700 shadow-2xl shadow-emerald-500/20"
                           >
                             Initiate New Request <PlusCircle className="w-6 h-6" />
                           </button>
                        </div>
                        <div className="absolute top-10 right-10 opacity-10"><Zap className="w-64 h-64" /></div>
                     </div>
                     <div className="premium-card p-12 border-dashed flex flex-col items-center justify-center text-center space-y-6">
                        <div className="w-20 h-20 bg-zinc-50 rounded-[2rem] flex items-center justify-center"><Sparkles className="w-10 h-10 text-black/20" /></div>
                        <p className="text-label">Project Intelligence</p>
                        <p className="text-xs text-gray-400 italic">Advanced analytics for your active requests will appear here as you gather data.</p>
                     </div>
                   </div>
                 )}
                                 ...
                                 <div key={p.id} className="flex items-center justify-between p-10 bg-zinc-50/50 hover:bg-zinc-50 rounded-[3rem] transition-all duration-700 border border-transparent hover:border-black/5 group">
                                    <div className="flex items-center gap-8">
                                       ...
                                       <div>
                                          ...
                                          <p className="text-xs text-gray-400 font-medium italic mt-1 max-w-xl">"{p.coverLetter}"</p>
                                       </div>
                                    </div>
                                    <button 
                                      onClick={() => {
                                      ...
                                      className="flex items-center gap-4 px-10 py-5 bg-black text-white rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-emerald-500 transition-all duration-700 shadow-2xl shadow-black/20"
                                    >
             ...
             {activeTab === 'chats' && (
               <motion.div key="chats" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="h-[calc(100vh-16rem)] flex gap-10 overflow-hidden">
                  <div className="w-96 flex flex-col gap-6 overflow-y-auto pr-4 hide-scrollbar">
                     {activeChats.length === 0 ? (
                       <div className="p-20 text-center premium-card border-dashed opacity-40">
                          <MessageSquare className="w-16 h-16 mx-auto mb-8 text-black/10" />
                       ...
                       activeChats.map(chat => (
                         <button 
                           ...
                           className={`p-10 premium-card text-left relative overflow-hidden group transition-all duration-700 ${activeChat?.id === chat.id ? 'border-black ring-4 ring-black/5 shadow-2xl scale-[1.02]' : 'hover:border-black/20'}`}
                         >
                            <div className="flex justify-between items-start mb-4">
                               <span className="text-[9px] font-black uppercase tracking-widest px-4 py-1.5 bg-emerald-500 text-white rounded-full shadow-lg shadow-emerald-500/20">Live Contract</span>
                            ...
                            <div className="flex items-center gap-3 mt-6 pt-6 border-t border-black/5">
                               <div className="w-8 h-8 bg-zinc-100 rounded-lg flex items-center justify-center font-black text-xs">{user.role === 'client' ? 'F' : 'C'}</div>
                  ...
                  <div className="flex-1 premium-card border-black/10 flex flex-col overflow-hidden bg-white/40 backdrop-blur-3xl">
                     {activeChat ? (
                       <>
                         <div className="px-12 py-10 border-b border-black/5 flex items-center justify-between bg-white/50">
                            <div className="flex items-center gap-6">
                               <div className="w-14 h-14 bg-black rounded-[1.25rem] flex items-center justify-center text-white font-black text-2xl rotate-3">
                                  {user.role === 'client' ? 'S' : 'C'}
                               </div>
                               <div>
                                  <h3 className="text-display text-2xl italic">{activeChat.title}</h3>
                                  <div className="flex items-center gap-3 mt-1">
                                     <div className="w-2 h-2 bg-emerald-500 rounded-full"></div>
                                     <p className="text-label text-[8px]">Encrypted Secure Tunnel Established</p>
                                  </div>
                               </div>
                            </div>
                            <div className="flex gap-4">
                               {user.role === 'client' && (
                                  <button 
                                    ...
                                    className="px-8 py-4 bg-emerald-500 text-white rounded-full hover:scale-105 transition-all text-[10px] font-black uppercase tracking-widest flex items-center gap-3 shadow-xl shadow-emerald-500/20"
                                  >
                                    <CheckCircle2 className="w-4 h-4" /> Finalize Delivery
                                  </button>
                               )}
                               <button className="w-12 h-12 rounded-full border border-black/5 flex items-center justify-center hover:bg-black hover:text-white transition-all duration-500"><Ban className="w-4 h-4" /></button>
                               <button className="w-12 h-12 rounded-full border border-black/5 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all duration-500 text-red-500"><Trash2 className="w-4 h-4" /></button>
                            </div>
                         </div>
                         ...
                         <div className="flex-1 p-16 overflow-y-auto flex flex-col gap-10">
                            <div className="flex flex-col items-center justify-center py-20 text-center space-y-8 opacity-20 group">
                               <div className="w-24 h-24 bg-zinc-50 rounded-[3rem] flex items-center justify-center rotate-3 group-hover:rotate-0 transition-transform duration-1000"><Shield className="w-12 h-12" /></div>
      ...
      <aside className="w-96 border-l border-black/5 h-screen overflow-y-auto p-12 space-y-10 bg-white/20 backdrop-blur-xl relative z-10 sticky top-0 shrink-0">
         <div className="premium-card p-12 flex flex-col items-center text-center space-y-10 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-48 h-48 bg-zinc-50 rounded-full -mr-24 -mt-24 group-hover:scale-125 transition-transform duration-[5s]"></div>
            
            <div className="w-32 h-32 bg-black rounded-[3rem] flex items-center justify-center text-white text-5xl font-black italic rotate-6 shadow-2xl relative z-10 group-hover:rotate-0 transition-transform duration-700">
               {user.fullName[0]}
            </div>

            <div className="space-y-4 relative z-10">
               <h3 className="text-display text-3xl leading-none">{user.fullName}</h3>
               <div className="flex items-center justify-center gap-3">
                  <div className="px-5 py-1.5 bg-emerald-500 text-white rounded-full text-[8px] font-black uppercase tracking-widest shadow-xl shadow-emerald-500/20">Verified Identity</div>
               </div>
            </div>

            <div className="grid grid-cols-2 w-full gap-8 border-t border-black/5 pt-10 relative z-10">
               <div className="text-center space-y-2">
                  <p className="text-display text-4xl">{user.role === 'freelancer' ? `${(user.rating || 5.0).toFixed(1)}` : (user.postedJobsCount || 0)}</p>
                  <p className="text-label text-[8px] text-black/30 leading-none">{user.role === 'freelancer' ? 'Rating Index' : 'Outbound'}</p>
               </div>
               <div className="text-center space-y-2 border-l border-black/5">
                  <p className="text-display text-4xl">{user.completedJobsCount || 0}</p>
                  <p className="text-label text-[8px] text-black/30 leading-none">Milestones</p>
               </div>
            </div>

            <div className="w-full space-y-4 pt-4 relative z-10">
               <div className="flex justify-between items-end">
                 <span className="text-label text-[7px] text-black/40">Authority Level</span>
                 <span className="text-label text-black/80">98%</span>
               </div>
               <div className="h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                  <motion.div initial={{ width: 0 }} animate={{ width: '98%' }} transition={{ duration: 2 }} className="h-full bg-black"></motion.div>
               </div>
            </div>
         </div>
      ...
      <AnimatePresence>
        {isCreatingJob && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-12">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/90 backdrop-blur-3xl" onClick={() => setIsCreatingJob(false)} />
            <motion.div 
              initial={{ scale: 0.95, opacity: 0, y: 50 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.95, opacity: 0, y: 50 }}
              transition={{ type: 'spring', damping: 25 }}
              className="bg-white rounded-[5rem] p-24 w-full max-w-5xl relative z-10 flex flex-col md:flex-row gap-20 overflow-hidden"
            >
               ...
               <div className="w-80 space-y-12 shrink-0">
                  <div className="premium-card p-12 bg-black text-white space-y-8 h-fit">
                     <Command className="w-12 h-12 text-emerald-500" />
                     <h4 className="text-display text-3xl italic">Audit Protocol</h4>
                     <p className="text-[10px] text-zinc-400 leading-relaxed font-medium">All projects are manually audited for compliance. Ensure your brief is explicit and clear.</p>
                     <ul className="space-y-4 pt-6 border-t border-white/10">
                        <li className="flex items-center gap-3 text-[9px] font-black uppercase tracking-widest text-emerald-400"><CheckCircle2 className="w-4 h-4" /> Secure Escrow</li>
                        <li className="flex items-center gap-3 text-[9px] font-black uppercase tracking-widest text-emerald-400"><CheckCircle2 className="w-4 h-4" /> Priority Discovery</li>
                        <li className="flex items-center gap-3 text-[9px] font-black uppercase tracking-widest text-emerald-400"><CheckCircle2 className="w-4 h-4" /> Expert Matching</li>
                     </ul>
                  </div>
                  ...
                  <div className="space-y-6">
                     ...
                     <button onClick={() => setIsCreatingJob(false)} className="w-full text-label hover:text-black transition-colors">Abort Procedure</button>
                  </div>
               </div>
            </motion.div>
          </div>
...
const SidebarLink = ({ active, icon: Icon, onClick, label, badge }: any) => (
...
const ModernJobCard = ({ job, user, applyToJob, hasApplied }: any) => (
    ...
    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-12 relative z-10">
      ...
      <div className="text-left lg:text-right flex flex-col items-start lg:items-end gap-8 min-w-[250px]">
         ...
         {user.role === 'freelancer' && (
           <button 
             onClick={() => !hasApplied && applyToJob({ jobId: job.id, freelancerId: user.id, freelancerName: user.fullName, bid: job.budget, coverLetter: "Proposal transmitted. Expert alignment confirmed." })}
             disabled={hasApplied}
...
const StatRow = ({ label, value, color = 'text-black' }: any) => (
