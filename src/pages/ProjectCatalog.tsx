    ...
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-20">
      <header className="space-y-12">
        <div className="space-y-4">
           <h1 className="text-display text-8xl italic leading-none bg-gradient-to-r from-white via-white to-white/20 bg-clip-text text-transparent uppercase text-center">{t('mission_registry')}</h1>
           <p className="text-label text-[8px] tracking-[0.5em] text-white/20 uppercase text-center">{t('registry_subtitle')}</p>
        </div>
      </header>
        ...
        <aside className="w-full lg:w-80 space-y-10 shrink-0">
           <div className="premium-card p-8 bg-white/[0.02] border-white/5 space-y-10">
              <div className="space-y-6">
                 <h4 className="text-label text-violet-400 tracking-[0.4em]">{t('project_protocol')}</h4>
                 <div className="space-y-3">
                    <FilterCheck label={t('fixed_price')} />
                    <FilterCheck label={t('hourly_sync')} />
                    <FilterCheck label={t('retainer_node')} />
                 </div>
              </div>

              <div className="space-y-6">
                 <h4 className="text-label text-fuchsia-400 tracking-[0.4em]">{t('complexity_level')}</h4>
                 <div className="space-y-3">
                    <FilterCheck label={t('expert_arch')} />
                    <FilterCheck label={t('standard_impl')} />
                    <FilterCheck label={t('entry_task')} />
                 </div>
              </div>
              ...
              <button className="w-full bg-white text-black py-4 text-[9px] font-bold uppercase tracking-widest hover:bg-violet-500 hover:text-white transition-colors">{t('scan_registry')}</button>
           </div>
        </aside>
           ...
           <div className="grid grid-cols-1 gap-8">
              {jobs.length === 0 ? (
                <div className="py-40 premium-card bg-white/[0.01] border-dashed border-white/10 text-center space-y-8">
                   ...
                   <p className="text-label text-white/10 tracking-[0.4em]">{t('no_missions')}</p>
                </div>
              ) : (
                jobs.map(job => (
                  <motion.div 
                  ...
                  >
                     <div className="flex-1 space-y-6">
                        <div className="flex items-center gap-4">
                           ...
                           <span className="text-label text-[8px] text-white/10 uppercase tracking-widest">{job.type} {t('project_protocol')}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-8 pt-4">
                           ...
                           <div className="flex items-center gap-2 text-violet-400 text-[9px] font-bold uppercase tracking-widest"><Users className="w-4 h-4" /> {job.proposalsCount} {t('proposals_received')}</div>
                        </div>
                     </div>
                     <div className="text-right space-y-6 shrink-0">
                        <div className="space-y-1">
                           ...
                           <p className="text-label text-[8px] text-white/20 uppercase tracking-widest">{t('resource_allocation')}</p>
                        </div>
                        <Link to={`/job/${job.id}`} className="btn-lux px-10 py-5 text-[10px]">{t('audit_objective')} <ArrowUpRight className="w-5 h-5" /></Link>
                     </div>
                  </motion.div>
                ))
              )}
